...
interface Props {
  subdomain: string;
  initialData: { success: boolean; agent: Agent; packages: Package[] } | null;
}
...
/* ─── Package Card ─────────────────────────────────────────── */
function PackageCard({ pkg, theme, agentWhatsapp }: { pkg: Package; theme: typeof THEMES[string]; agentWhatsapp: string }) {
  ...
  return (
    <div style={{ borderRadius: 20, overflow: 'hidden', background: '#fff', boxShadow: '0 4px 24px rgba(0,0,0,0.08)', transition: 'transform 0.25s, box-shadow 0.25s', display: 'flex', flexDirection: 'column' }}
      className="pkg-card">
      {/* Image */}
      <div style={{ position: 'relative', height: 210, background: '#f1f5f9', overflow: 'hidden' }}>
        {pkg.imageUrl1 ? (
          <img src={pkg.imageUrl1} alt={pkg.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ) : (
          <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 64, background: 'linear-gradient(135deg,#f8fafc,#e2e8f0)' }}>
...
export default function AgentMarketplaceClient({ subdomain, initialData }: Props) {
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f8fafc', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
        <div style={{ textAlign: 'center', padding: 40 }}>
          ...
          <h1 style={{ fontSize: 28, fontWeight: 800, color: '#0f172a', marginTop: 16 }}>Agent Not Found</h1>
          <p style={{ color: '#64748b', marginTop: 8 }}>No marketplace found for <strong>{subdomain}</strong>.localhost</p>
          <a href="http://localhost:3000" style={{ display: 'inline-block', marginTop: 24, background: '#7c3aed', color: '#fff', padding: '12px 28px', borderRadius: 12, textDecoration: 'none', fontWeight: 700 }}>
            Go to Orbitle
          </a>
        </div>
      </div>
  ...
  const { agent, packages } = initialData;
  ...
  const logoUrl = agent.logo && agent.logo !== 'no-photo.jpg'
    ? `http://localhost:5000/uploads/${agent.logo}`
    : null;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc', fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
      ...
      <div style={{ background: theme.gradient, position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="hero-section" style={{ maxWidth: 1100, margin: '0 auto', padding: '56px 24px 48px', position: 'relative', zIndex: 1 }}>
          {/* Logo + Name */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginBottom: 24 }}>
            <div style={{ width: 72, height: 72, borderRadius: 18, background: 'rgba(255,255,255,0.15)', border: '2px solid rgba(255,255,255,0.3)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(8px)', flexShrink: 0 }}>
              {logoUrl ? (
            ...
            <div>
              ...
              <h1 className="hero-title" style={{ margin: 0, fontSize: 32, fontWeight: 800, color: '#fff', lineHeight: 1.2 }}>{displayName}</h1>
            </div>
          </div>

          {agent.tagline && (
            <p style={{ fontSize: 17, color: 'rgba(255,255,255,0.85)', maxWidth: 540, lineHeight: 1.6, margin: '0 0 28px' }}>
              {agent.tagline}
            </p>
          )}
          ...
          <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
            <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 12, padding: '12px 20px', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.2)' }}>
              <div style={{ fontSize: 22, fontWeight: 800, color: '#fff' }}>{packages.length}</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)', fontWeight: 500 }}>Packages</div>
            </div>
            <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 12, padding: '12px 20px', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.2)' }}>
              <div style={{ fontSize: 22, fontWeight: 800, color: '#fff' }}>{packages.filter(p => p.category === 'domestic').length}</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)', fontWeight: 500 }}>Domestic</div>
            </div>
            <div style={{ background: 'rgba(255,255,255,0.12)', borderRadius: 12, padding: '12px 20px', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.2)' }}>
              <div style={{ fontSize: 22, fontWeight: 800, color: '#fff' }}>{packages.filter(p => p.category === 'international').length}</div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)', fontWeight: 500 }}>International</div>
            </div>
            {whatsappLink && (
              <a href={whatsappLink} target="_blank" rel="noopener noreferrer"
      ...
      {/* ── Footer ── */}
      <footer style={{ borderTop: '1px solid #e2e8f0', padding: '24px', textAlign: 'center', background: '#fff', marginTop: 40 }}>
        <p style={{ margin: 0, fontSize: 13, color: '#94a3b8' }}>
          Powered by{' '}
          <a href="http://localhost:3000" style={{ color: theme.primary, fontWeight: 700, textDecoration: 'none' }}>Orbitle</a>
